%% Obj_F Object Function

%**************************************************************************
% Caculate the Object Function K                                     Mr.Guo
% K = Sigma|S21(omigaS21)|*|S21(omigaS21)| + Sigma|S11(omigaS11)|*|S11(omigaS11)|
% Input:
...
function K = Obj_F(x, S21, S11, S22, Freq, delta, M, Opti_Var, W, CF, FBW)
% A = [4,2;2,2]; b = [1;-1];
Ns = length(S11);
K = 0;
K11 = 0; K21 = 0; K22 = 0;
...
for index = 1:Ns
    ...
    Exp21 = exp(1i*Fai21);
    K21 = K21+0.5*W(index)*abs(Exp21*S21_M-S21(index))*abs(Exp21*S21_M-S21(index));

    S11_M = -1+2*invA(1,1);

    Fai11 = L(1)*TransX + L(1)*TransX + Theta(1) + Theta(1);
    Exp11 = exp(1i*Fai11);
    K11 = K11+0.5*W(index+Ns)*abs(Exp11*S11_M-S11(index))*abs(Exp11*S11_M-S11(index));
    ...
    Exp22 = exp(1i*Fai22);
    K22 = K22+0.5*W(index+Ns*2)*abs(Exp22*S22_M-S22(index))*abs(Exp22*S22_M-S22(index));
